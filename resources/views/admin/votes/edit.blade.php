<x-app-layout>
    <x-slot name="header">
        <div class="p-6 rounded overflow-hidden shadow-lg border">
            ...
            <form class="w-full" action="{{ route('admin.votes.update', $vote) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="flex flex-col md:flex-row -mx-3">
                    <div class="w-full md:w-1/2 px-3 md:mb-6">
                        ...
                        <input
                            ...
                            id="titleInput" type="text" name="title" value="{{ $vote->title }}">
                        @error('title')
                        ...
                        <div class="w-full md:w-1/2 px-3 md:mb-6">
                            ...
                            <div class="form-check">
                                <input class="form-check-input appearance-none h-4 w-4 border border-gray-300 rounded-sm bg-white checked:bg-blue-600 checked:border-blue-600 focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer"
                                    id="groupInput" type="checkbox" name="group_id[]" value="{{ $group->id }}" @if ($vote->groups->contains('id', $group->id)) checked @endif>
                ...
                <div class="flex flex-col md:flex-row -mx-3">
                    <div class="w-full md:w-1/2 px-3 md:mb-6">
                        ...
                        <textarea
                            ...
                            id="descriptionInput" type="text" name="description">{{ $vote->description }}</textarea>
                        @error('description')
                        <span class="text-red-600">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="flex flex-col md:flex-row -mx-3">
                    <div class="w-full md:w-1/3 px-3 md:mb-6">
                        ...
                        <input
                            ...
                            id="startDateInput" type="datetime-local" name="start_date" value="{{ $vote->start_date }}">
                        @error('start_date')
                        <span class="text-red-600">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="w-full md:w-1/3 px-3">
                        ...
                        <input
                            ...
                            id="endDateInput" type="datetime-local" name="end_date" value="{{ $vote->end_date }}">
                        @error('end_date')
                        <span class="text-red-600">{{ $message }}</span>
                        @enderror
                    </div>
